...
public class QuartzConfigGongan {
    ...
    @Resource
    private HomeAttentionTask homeAttentionTask;
    ...
    @Resource
    CaptureDeleteTask captureDeleteTask;
    /**
     * 功能描述: 定时删除三个月之前的抓拍数据
     * @auther: lixianhua
    ...
    @Async
    public void setCaptureDeleteTaskTaskQuartz() {
        try {
            captureDeleteTask.run();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
