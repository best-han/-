...
public class TokenController extends BaseController {
    ...
    public Map<String,Object> login(@RequestBody Map<String, Object> params) {
        ...
        String ifManage = Tools.null2String((String)params.get("ifManage"));//是否进入用户管理页面（1：是）
        try {
   ...
   /*         if(!zuulKaptcha.validate(verifyCode, 300)){
            ...
            if (tokenInfo != null && !StringUtils.isBlank(tokenInfo.get("access_token").toString())) {
                ...
                data.put("subdistrictName", ((Map<String, Object>)tokenInfo.get("loginUser")).get("subdistrictName"));
                //用于判断是否为管理员身份
                String sysLevel=(String)((Map<String, Object>)tokenInfo.get("loginUser")).get("sysLevel");
                data.put("sysLevel", ((Map<String, Object>)tokenInfo.get("loginUser")).get("sysLevel"));
                if("1".equals(ifManage) && sysLevel.equals("3")){//普通用户不能登陆管理用户系统
